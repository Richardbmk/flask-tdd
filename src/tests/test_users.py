...
def test_add_user(test_app, test_database):
    ...
    resp = client.post(
        "/users",
        data=json.dumps({"username": "richard", "email": "richard@testdriven.io"}),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
    assert resp.status_code == 201
    assert "richard@testdriven.io was added!" in data["message"]
...
def test_add_user_invalid_json(test_app, test_database):
    ...
    def test_add_user_invalid_json_keys(test_app, test_database):
        ...
        resp = client.post(
            "/users",
            data=json.dumps({"email": "mike@testdriven.io"}),
            content_type="application/json",
        )
        data = json.loads(resp.data.decode())
        assert resp.status_code == 400
...
def test_single_user(test_app, test_database, add_user):
    user = add_user(username="adams", email="adams@testdriven.io")
    client = test_app.test_client()
    resp = client.get(f"/users/{user.id}")
    data = json.loads(resp.data.decode())
    assert resp.status_code == 200
    assert "adams" in data["username"]
    assert "adams@testdriven.io" in data["email"]
...
def test_update_user(test_app, test_database, add_user):
    user = add_user("user-to-be-update", "update-me@testdriven.io")
    client = test_app.test_client()
    resp_one = client.put(
        f"/users/{user.id}",
